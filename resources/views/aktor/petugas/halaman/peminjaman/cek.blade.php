                    ...
                    <ul class="px-3">
                        <li class="fs-16 mb-1">
                            <span class="fs-15">
                                ...
                                Perpustakaan Untuk Bisa Melakukan Peminjaman Buku
                            </span>
                        </li>
                        <li class="fs-16 mb-1">
                            <span class="fs-15">
                        ...
                        @foreach ($denda as $data)
                            ...
                            <li class="fs-16 mb-1">
                                <span class="fs-15">
                                    Apabila Buku Hilang Maka Siswa Diberlakukan denda Rp.{{ number_format($data->denda_buku_hilang) }} / Buku
                                </span>
                            </li>
                            <li class="fs-16 mb-1">
                                <span class="fs-15">
                                    Apabila Buku Rusak Maka Siswa Diberlakukan denda Rp.{{ number_format($data->denda_buku_rusak) }} / Buku
                                </span>
                            </li>
                        @endforeach
                    </ul>
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <form method="POST" action="{{ route('petugas.peminjamanStore') }}" enctype="multipart/form-data">
                            @csrf
                            <input type="hidden" name="siswa" class="form-controler" value="{{ $siswa->id }}">
                            <div class="row">
                                <div class="col-12 mb-0">
                                    <label for="buku1" class="col-form-label">Judul Buku yang ingin dipinjam:</label>
                                    <select name="buku1" id="buku1" class="form-control select2">
                                        <option value="">Daftar Buku</option>
                                        @foreach ($buku as $data)
                                            <option value="{{ $data->id }}" data-title="{{ $data->judul }}" data-stok="{{ $data->stok_buku }}">
                                                {{ $data->judul }} ({{ $data->tahun_terbit }}) - (Stok Buku: {{ $data->stok_buku }})
                                            </option>
                                        @endforeach
                                    </select>
...
<script>
    $(document).ready(function() {
        function updateBukuOptions() {
            var selectedBuku1 = $('#buku1').val();
            var selectedBuku2 = $('#buku2').val();
            var selectedBuku3 = $('#buku3').val();

            $('#buku2 option').prop('disabled', false);
            $('#buku3 option').prop('disabled', false);

            $('#buku2 option').each(function() {
                if ($(this).val() == selectedBuku1 || $(this).val() == selectedBuku3) {
                    $(this).prop('disabled', true);
                }
            });
            $('#buku3 option').each(function() {
                if ($(this).val() == selectedBuku1 || $(this).val() == selectedBuku2) {
                    $(this).prop('disabled', true);
                }
            });
        }

        // Ketika nilai buku pertama dipilih
        $('#buku1').change(function() {
            updateBukuOptions();
        });

        // Ketika nilai buku kedua dipilih
        $('#buku2').change(function() {
            updateBukuOptions();
        });

        // Ketika nilai buku ketiga dipilih
        $('#buku3').change(function() {
            updateBukuOptions();
        });

        // Panggil sekali untuk inisialisasi saat halaman pertama kali dimuat
        updateBukuOptions();
    });
</script>
@endsection

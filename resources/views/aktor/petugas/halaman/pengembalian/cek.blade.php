                                        ...
                                        @if (count($pinjam) > 0)
                                            @foreach ($pinjam as $row)
                                                @php
                                                    ...
                                                    $nominalDenda = $hariTerlambat > 0 ? $row->getNominalDenda() : 0;
                                                @endphp
                                                <tr>
                                                    <td>
                                                    ...
                                                    <td>
                                                        @if ($hariTerlambat > 0)
                                                            <span class="badge bg-danger text-light">Terlambat {{ $hariTerlambat }} Hari</span>
                                                        @else
                                                            <span class="badge bg-info text-light">Tidak Denda</span>
                                                        @endif
                                                    </td>
                                                    <td>
                                                        {{ $nominalDenda > 0 ? 'Rp ' . number_format($nominalDenda) : 'Rp 0,00' }}
                                                    </td>
                                                    <td>
                                                        {{-- <div class="d-flex justify-content-between">
                                                            ...
                                                            <form method="POST" action="{{ route('petugas.pengembalianBukuRusak', $row->id) }}" id="bukuRusak-form-{{ $row->id }}">
                                                            @csrf <input type="hidden" name="id" value="{{ $row->id }}">
                                                            <input type="hidden" name="denda_buku_rusak" value="{{ $denda_buku_rusak }}">
                                                                <a href="javascript: void(0);" onclick="confirmBukuRusak('{{ $row->id }}')" class="text-reset fs-18 px-1" title="Denda Buku Rusak">
                                                                    <i class="ri-copper-coin-line"></i>
                                                                </a>
                                                            </form>
                                                            <form method="POST" action="{{ route('petugas.pengembalianBukuHilang', $row->id) }}" id="bukuHilang-form-{{ $row->id }}">
                                                            @csrf <input type="hidden" name="id" value="{{ $row->id }}">
                                                            <input type="hidden" name="denda_buku_hilang" value="{{ $denda_buku_hilang }}">
                                                                <a href="javascript: void(0);" onclick="confirmBukuHilang('{{ $row->id }}')" class="text-reset fs-18 px-1" title="Denda Buku Hilang">
                                                            ...
                                                            <div class="dropdown">
                                                                ...
                                                                <div class="dropdown-menu dropdown-menu-end">
                                                                    <form method="POST" action="{{ route('petugas.pengembalianProses', $row->id) }}" id="proses-form-{{ $row->id }}">
                                                                        @csrf <input type="hidden" name="denda" value="{{ $hariTerlambat > 0 ? $hariTerlambat : 0 }}">
                                                                        <input type="hidden" name="nominal" value="{{ $nominalDenda }}"> <input type="hidden" name="id" value="{{ $row->id }}">
                                                                        <a href="javascript:void(0);" onclick="confirmProses('{{ $row->id }}')" class="dropdown-item">Proses</a>
                                                                    </form>
                                                                    <form method="POST" action="{{ route('petugas.pengembalianBukuRusak', $row->id) }}" id="bukuRusak-form-{{ $row->id }}">
                                                                        @csrf <input type="hidden" name="id" value="{{ $row->id }}">
                                                                        <input type="hidden" name="denda_buku_rusak" value="{{ $denda_buku_rusak }}">
                                                                        <a href="javascript:void(0);" onclick="confirmBukuRusak('{{ $row->id }}')" class="dropdown-item">Proses Buku Rusak</a>
                                                                    </form>
                                                                    <form method="POST" action="{{ route('petugas.pengembalianBukuHilang', $row->id) }}" id="bukuHilang-form-{{ $row->id }}">
                                                                    @csrf <input type="hidden" name="id" value="{{ $row->id }}">
                                                                        <input type="hidden" name="denda_buku_hilang" value="{{ $denda_buku_hilang }}">
                                                                        <a href="javascript:void(0);" onclick="confirmBukuHilang('{{ $row->id }}')" class="dropdown-item">Proses Buku Hilang</a>
                                                                    </form>
                                                                </div>
                                                            </div>
                                                        </div>
...
@section('script')
    ...
    <script>
        function confirmBukuRusak(id) {
            Swal.fire({
                title: 'Apakah Anda yakin?',
                text: "Apakah buku rusak ?",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonText: 'Ya, proses!',
                cancelButtonText: 'Batal'
    ...
    <script>
        function confirmBukuHilang(id) {
            Swal.fire({
                title: 'Apakah Anda yakin?',
                text: "Apakah buku hilang ?",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonText: 'Ya, proses!',
                cancelButtonText: 'Batal'

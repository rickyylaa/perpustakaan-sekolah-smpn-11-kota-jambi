                                ...
                                <tbody>
                                    @if (count($rakBaris) > 0)
                                        @php
                                            $rowNumber = 1;
                                        @endphp
                                        @foreach ($rakBaris as $row)
                                            <tr>
                                                <td>{{ $rowNumber }}</td>
                                                <td>{{ ucwords($row->kategori->nama) }}</td>
                                                <td>{{ $row->rak }}</td>
                                                <td>{{ $row->baris }}</td>
                                                <td class="text-center">
                                                    <form method="POST" action="{{ route('petugas.rakBarisDestroy', $row->id) }}" id="delete-form-{{ $row->id }}">
                                                        @csrf @method('DELETE')
                                                    ...
                                                    <div class="modal-dialog modal-dialog-centered modal-md">
                                                        <div class="modal-content">
                                                            <div class="modal-header modal-colored-header bg-dark">
                                                                <h4 class="modal-title" id="editModalLabel">Edit Rak dan Baris</h4>
                                                                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                                                            </div>
                                                            <form method="POST" action="{{ route('petugas.rakBarisUpdate', $row->id) }}" enctype="multipart/form-data">
                                                                @csrf @method('PUT')
                                                                <div class="modal-body">
                                                                    <div class="row">
                                                                        <div class="col-12">
                                                                            ...
                                                                            <select name="kategori_id" id="kategori_id" class="form-control">
                                                                                ...
                                                                                @foreach ($kategori as $data)
                                                                                    <option value="{{ $data->id }}" {{ (($row->kategori_id == $data->id) ? 'selected' : '') }}>{{ $data->nama }}</option>
                                                                                @endforeach
                                                                            </select>
                                                                        ...
                                                                        <div class="col-md-6">
                                                                            ...
                                                                            <input type="number" name="rak" id="rak" class="form-control" value="{{ old('rak', $row->rak) }}" placeholder="Masukan rak" required>
                                                                            <p class="text-danger fs-16">{{ $errors->first('rak') }}</p>
                                                                        </div>
                                                                        <div class="col-md-6">
                                                                            <label for="baris" class="form-label">Baris</label>
                                                                            <input type="number" name="baris" id="baris" class="form-control" value="{{ old('baris', $row->baris) }}" placeholder="Masukan baris" required>
...
@section('modal')
    <div class="modal fade" id="tambahModal" tabindex="-1" role="dialog" aria-labelledby="tambahModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-md">
            <div class="modal-content">
                <div class="modal-header modal-colored-header bg-dark">
                    <h4 class="modal-title" id="tambahModalLabel">Tambah Rak Dan Baris</h4>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form method="POST" action="{{ route('petugas.rakBarisStore') }}" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-12">
